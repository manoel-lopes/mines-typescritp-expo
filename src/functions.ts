...
const openField = (board: Board[][], row: number, column: number) => {
  ...
  if (!field.opened) {
    ...
    if (field.mined) {
      ...
      field.nearMines = neighborsOnField.filter(neighbors => 
        neighbors.mined).length
    }
  }
}


const getFields = (board: Board[][]): Field[] => [].concat(...board)
...
const wonGame = (board: Board[][]) =>
  getFields(board).filter(getPendingFields).length === 0
...
const getUsedFlags = (board: Board[][]) =>
